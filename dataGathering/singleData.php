...
try {
	...
	$stmt= $dbh->prepare("SELECT userID, fullname, bannerID, travelMode FROM userData;"); // prepare for execution & return a statement object
	...
	echo "<table><tr><th>User ID</th><th>Banner ID</th><th></th></tr>";
    while ( $stmt->fetch(PDO::FETCH_BOUND) ){ // fetch one row of results binding coulmn values to PHP variables
		echo "<tr><td>$userIDSel</td><td>$bannerIDSel</td><td>";
        echo "<form action='selectSingleData.php' method='GET'>";
