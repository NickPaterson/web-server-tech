...
<body class="dark">
    ...
    <h1 class="page-title">Data Gathering - Update Single Data</h1> 
...
try {
	...
	$stmt->bindColumn( 'fullname', $fullname ); // Bind named column to a named PHP variable
	$stmt->bindColumn( 'bannerID', $bannerID ); 
    $stmt->bindColumn( 'travelMode', $travelMode );
	$stmt->execute();
    ...
    while ( $stmt->fetch(PDO::FETCH_BOUND) ){ // fetch one row of results binding coulmn values to PHP variables
		echo "<form action='updateUserData.php' method='POST'>";
        echo "<input type='hidden' name='userID' value='$userID'>";
        echo "<label for='bannerID'>Banner ID:</label>";
        echo "<input id='bannerID' name='bannerID' placeholder='$bannerID' value='$bannerID'>"; 
        echo "<br>";

        echo "<label for='fullname'>Fullname:</label>";
        echo "<input id='fullname' name='fullname' placeholder='$fullname' value='$fullname'>";
        echo "<br>";

        echo "<label for='travelMode'>Travel Mode:</label>";
        echo "<select name='travelMode'>";
        echo "    <option value='Car'";
        if ($travelMode == "Car") { echo "selected";}
        echo ">Car</option>";
        echo "    <option value='Bus'";
         if ($travelMode == "Bus") { echo "selected";}
         echo ">Bus</option>";
         echo "    <option value='Train'";
         if ($travelMode == "Train") { echo "selected";}
         echo ">Train</option>";
         echo "    <option value='Walking'";
         if ($travelMode == "Walking") { echo "selected";}
         echo ">Walking</option>";
         echo "</select>";
